import React, { Component} from 'react'
import { connect } from 'react-redux'
import {loginCustomer,logoutCustomer} from '../Actions'
import {Redirect} from 'react-router-dom'
import TeamMessage from '../Components/TeamMessage'
import {getCustomer} from "../Actions/index";

class TeamMessageContainer extends Component {
    constructor(props) {
        super(props)
//    this.handleUsername = this.handleUsername.bind(this);
//    this.handlePassword = this.handlePassword.bind(this);
        this.state = {
            username: this.props.username,
            password: '',
            open: false,
            error: {
                username: '',
                password: '',
            },
            navigate: 'false',
        }
        console.log("logggg ::: " + this.props.username)
    }
    //
    componentWillMount() {
        //this.props.loginCustomer;
        console.log("componentWillMount this.props.username "+this.props.username);
    }

    render() {
        console.log("TeamMessageContainer render " + this.props.state.username);
        const { loginCustomer,isAuthenticated } = this.props;
        let url;
        return(
            <TeamMessage //history={this.props.history}
                loginCustomer={loginCustomer}
                username = {this.props.username}

            />
        );
    }
}

const mapStateToProps = (state) => ({//定义怎么绑定
    isAuthenticated: state.customer.isAuthenticated,//前者是TeamMessageContainer的是一个props, 它后来又传给了components／TeamMessage
                                        //后者是reducers/index.js里的一个函数
                                        ///后者是Reducers／Customer.js的state里的成员
    username:state.customer.username,
    state: state.customer,
});

export default connect(// 把需要绑定的东西放进去
    mapStateToProps,
)(TeamMessageContainer)